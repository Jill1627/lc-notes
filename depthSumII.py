...
class Solution(object):
    ...
    def depthSumHelper(self, nestedList, carry):
        intRes = carry
        listRes = 0
        nextLevel = list()
        for elem in nestedList:
            if elem.isInteger():
                intRes += elem.getInteger()
            else:
                nextLevel.extend(elem.getList())
        listRes = 0 if len(nextLevel) == 0 else self.depthSumHelper(nextLevel, intRes)
        return intRes + listRes
